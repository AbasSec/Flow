...
import type { PublicOutletMenu } from "@/lib/services/public-ordering";
import {
  submitOutletPublicOrderAction,
  type PublicOutletOrderActionState
} from "./actions";
...
export function PublicOutletMenuClient({ menu, outletToken }: { menu: PublicOutletMenu; outletToken: string }) {
  const [selectedTableToken, setSelectedTableToken] = useState("");
  const [cart, setCart] = useState<CartLine[]>([]);
  const [requestKey] = useState(() => crypto.randomUUID());
  const [state, formAction, isPending] = useActionState<
    PublicOutletOrderActionState,
    FormData
  >(submitOutletPublicOrderAction, { message: "" });

  const selectedTable = menu.tables.find(
    (t) => t.publicTableToken === selectedTableToken
  );
  ...
  const canSubmit = selectedTableToken !== "" && cart.length > 0;
  ...
  return (
    <form action={formAction} className="mt-5 space-y-5">
      <input name="outletToken" type="hidden" value={outletToken} />
      <input name="tableToken" type="hidden" value={selectedTableToken} />
      ...
      <div className="border border-[#d8d1c1] bg-white p-4 shadow-sm">
        ...
        {menu.tables.length === 0 ? (
          ...
          <>
            <select
              className="mt-3 w-full border border-[#c8c1b1] bg-[#faf9f4] px-3 py-2 text-sm text-[#17211b] outline-none focus:border-[#17211b] focus:ring-2 focus:ring-[#17211b]"
              id="table-select"
            ...
            >
              ...
              {menu.tables.map((table) => (
                <option key={table.publicTableToken} value={table.publicTableToken}>
                  {table.tableLabel}
                </option>
              ))}
            </select>
            {selectedTable && (
              <p className="mt-2 text-xs text-[#527245]">
                Table selected: {selectedTable.tableLabel}
              </p>
            )}
          </>
        )}
      ...
      <section className="sticky bottom-0 -mx-4 border-t border-[#d8d1c1] bg-[#f6f4ed]/95 px-4 py-4 backdrop-blur">
        <div className="border border-[#d8d1c1] bg-white p-4 shadow-sm">
          ...
          {!selectedTableToken && cart.length > 0 ? (
